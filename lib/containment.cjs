...
function selectCandidates(ev, c) {
  ...
  for (const w of (ev && ev.wallets) || []) {
    const o = w.out1hUsd || 0, inn = w.in24hUsd || 0;
    if (o < c.minUsd) continue;
    const entitlement = inn > 0 && o > 50 * inn; // the classic $1-in/$98k-out fingerprint (fast trigger)
    out.set(w.wallet, { wallet: w.wallet, out1hUsd: o, in24hUsd: inn, entitlement });
  }
  return [...out.values()];
}
...
async function verifyDrain(wallet, rpc, c, flashVaults) {
  const vaults = flashVaults instanceof Set ? flashVaults : new Set(flashVaults || []);
  const v = { wallet, proven: false, lifetimeIn: 0, lifetimeOut: 0, ratio: null, txCount: 0, capped: false, sigs: [], collateralMint: c.collateralMint, reason: "" };
  try {
    ...
    for (const s of list) {
      ...
      if (isWithdrawal) { v.lifetimeOut += d; if (v.sigs.length < 6) v.sigs.push(s.signature); }
      else if (isDeposit) { v.lifetimeIn += -d; }
...
function posture(c) {
  return {
    enabled: CONTAINMENT(),
    mode: MODE(),
    state: !CONTAINMENT() ? "DISARMED" : "ARMED (signal)",
    proofGated: true,
...
function buildAlarmText(v, c) {
  const wh = c.webhook ? `signal POSTed → ${(() => { try { return new URL(c.webhook).host; } catch { return "responder"; } })()}` : "NO responder webhook set (set CONTAINMENT_WEBHOOK_URL)";
  return (
    `🚨🚨 CONTAINMENT · FLASH V2 🚨🚨\n━━━━━━━━━━━━━━━━━━━━\nPROVEN OVER-WITHDRAWAL — DRAIN IN PROGRESS\n\n` +
    `Wallet: ${v.wallet}\n${v.reason}\nCollateral: USDC\n` +
    (v.sigs.length ? `Evidence: https://solscan.io/tx/${v.sigs[0]}${v.sigs.length > 1 ? ` (+${v.sigs.length - 1} more)` : ""}\n` : "") +
    `\nAUTOMATED RESPONSE:\n• Max-priority alert fired\n• Containment ${wh}\n• On-chain pause: signalled to Flash's authorized responder (the sentinel holds no pause key by design)\n\n` +
    `⚠️ PROOF-GATED: the wallet's ENTIRE ${v.txCount}-tx history was verified on-chain — this is not a threshold guess. Freeze this wallet / pause the affected vault NOW.`
  );
}
