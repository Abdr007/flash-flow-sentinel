...
function evaluate(nowUnix, events, failures, custodies, balances, marks, markTimes, lazerMarks, pyth, cfg, authority) {
  ...
  for (const e of events) {
    ...
    if (isOut) {
      const w = wal[e.wallet] || (wal[e.wallet] = { wallet: e.wallet, out1hUsd: 0, out24hUsd: 0, events: 0, tokens: new Set(), firstSeen: walletFirstSeen[e.wallet] != null ? walletFirstSeen[e.wallet] : e.blockTime });
      w.out24hUsd += usd || 0;
