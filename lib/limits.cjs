...
function ruleStates(ev) {
  ...
  for (const w of ev.wallets) {
    ...
    if ((w.out1hUsd || 0) >= 10000 && w.firstSeenAgoSec != null && w.firstSeenAgoSec <= 3600) signals.push(`fresh wallet (${Math.round(w.firstSeenAgoSec / 60)}m old)`);
    ...
    if (signals.length >= 2) {
      ...
      // dedup: fold the individual soft warns for this wallet into the one CRITICAL
