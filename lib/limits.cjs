...
// util == null → guard disabled/n-a → "ok". A non-finite util (NaN/Infinity from a malformed price or
// amount) must NOT read as "ok" — that would turn the verdict green on corrupt data; treat it as "warn".
const status = (util, warnFraction) => (util == null ? "ok" : !Number.isFinite(util) ? "warn" : util >= 1 ? "breach" : util >= warnFraction ? "warn" : "ok");
...
function evaluate(nowUnix, events, failures, custodies, balances, marks, markTimes, lazerMarks, pyth, cfg, authority) {
  ...
  // Per-wallet earliest appearance over the FULL retained buffer (not just the 24h window), plus the
  // buffer's coverage start. This makes "fresh wallet" mean genuinely new — an older wallet whose only
  // recent outflow is in the last hour is no longer mislabeled fresh — and lets the fresh-wallet rules
  // be suppressed until the buffer actually covers the freshness window (no false positives on warmup).
  const walletFirstSeen = {};
  let dataCoverageStart = nowUnix;
  for (const e of events) {
    if (e.blockTime == null || e.blockTime > nowUnix) continue; // ignore null / future-dated (clock skew)
    if (e.blockTime < dataCoverageStart) dataCoverageStart = e.blockTime;
    if (e.direction === "out" && e.wallet && (!authority || e.wallet !== authority)) {
      if (walletFirstSeen[e.wallet] == null || e.blockTime < walletFirstSeen[e.wallet]) walletFirstSeen[e.wallet] = e.blockTime;
    }
  }
  const dataCoverageSec = Math.max(0, nowUnix - dataCoverageStart);
  ...
  for (const e of events) {
    ...
    // per-wallet aggregation — track BOTH the wallet's withdrawals (vault→wallet, out) and its deposits
    // (wallet→vault, in), so the entitlement guard can compare "how much it took" vs "how much it put in".
    if (e.wallet && (!authority || e.wallet !== authority)) {
      const w = wal[e.wallet] || (wal[e.wallet] = { wallet: e.wallet, out1hUsd: 0, out24hUsd: 0, in1hUsd: 0, in24hUsd: 0, events: 0, tokens: new Set(), firstSeen: walletFirstSeen[e.wallet] != null ? walletFirstSeen[e.wallet] : e.blockTime, yield: false });
      if (isOut) { w.out24hUsd += usd || 0; if (in1h) { w.out1hUsd += usd || 0; w.events++; w.tokens.add(tkey(e)); } }
      else { w.in24hUsd += usd || 0; if (in1h) w.in1hUsd += usd || 0; }
      // mark yield-bearing wallets (LP / staking / reward / revenue / liquidity ops) — these are legitimate
      // yield collection, NOT deposit-then-drain probing, so they must never count toward the probe cluster.
      if (Array.isArray(e.ix) && e.ix.some((n) => /Liquidit|Stake|Reward|Revenue|Compound/i.test(n))) w.yield = true;
    }
  }
  ...
  const tokens = Object.values(tok).map((t) => {
    ...
    // TRUE drawdown = the vault's NET balance decline over 1h, NOT gross outflow. A high-churn settlement
    // asset (USDC) can push $28k out while taking $24k back in — its balance is essentially flat, so it is
    // NOT being drained. Measuring gross outflow falsely flags that as a "36% drawdown"; measuring the net
    // decline (6%) is the real drain signal. A drain masked by a matching deposit surfaces on the ENTITLEMENT
    // guard instead (that is what caught the $1-in/$98k-out exploit) — defense in depth, no blind spot.
    const netOutRaw = t.out1hRaw - t.in1hRaw; // BigInt; ≤ 0 when the vault held flat or grew
    const drawdownPct = balRaw1hAgo != null && balRaw1hAgo > 0n
      ? Math.max(0, (Number(netOutRaw) / Number(balRaw1hAgo)) * 100)
      : (netOutRaw > 0n ? 100 : 0);
    const grossOut1hPct = balRaw1hAgo != null && balRaw1hAgo > 0n ? r2((Number(t.out1hRaw) / Number(balRaw1hAgo)) * 100) : 0; // churn, informational only
    ...
    // drawdown only counts once the NET decline is material — a dust vault's big % on a tiny net move is
    // noise, not a drain (see drawdownMinUsd). A real vault-emptying clears this floor easily.
    const netOutUsd = t.out1hUsd - t.in1hUsd;
    const drawdownMaterial = netOutUsd >= (cfg.drawdownMinUsd || 0);
    ...
    return {
      ...
      drawdownPct1h: r2(drawdownPct), grossOut1hPct, drawdownLimitPct: cfg.vaultDrawdownPctPerHour,
  ...
  const wallets = Object.values(wal).map((w) => {
    ...
    return { wallet: w.wallet, watched: (cfg.watchWallets || []).includes(w.wallet), out1hUsd: r2(w.out1hUsd), out24hUsd: r2(w.out24hUsd), in1hUsd: r2(w.in1hUsd || 0), in24hUsd: r2(w.in24hUsd || 0), events: w.events, tokens: [...w.tokens], pctOfGlobal1h: out1.usd > 0 ? r2((w.out1hUsd / out1.usd) * 100) : null, firstSeenAgoSec: w.firstSeen != null ? Math.max(0, nowUnix - w.firstSeen) : null, limitUsdPerHour: wLimit, utilization: util == null ? null : r2(util * 100) / 100, status: status(util, cfg.warnFraction) };
  }).sort((a, b) => (b.out1hUsd || 0) - (a.out1hUsd || 0)).slice(0, 60); // wider than the display top-N so a dust-spread exfil (many small fresh recipients) can't hide below the cut

  // ---- probe-wallet detection (exploit-REHEARSAL fingerprint) ----
  // A "probe" ran a tiny deposit→withdraw round-trip (both legs small, net ≈ 0) on a recently-seen wallet —
  // testing the deposit/withdraw path WITHOUT extracting funds. One is noise; a CLUSTER appearing together is
  // the on-chain signature of someone rehearsing an exploit (exactly the pattern that preceded the live drain).
  // Computed over the FULL wallet set so a spread of tiny probes can't hide below the display cut.
  const PROBE_MAX = 100, PROBE_NET = 50, PROBE_FRESH = 21600; // ≤$100 per leg, net ≤$50, first seen ≤6h
  const probes = Object.values(wal).filter((w) => {
    const inn = w.in24hUsd || 0, out = w.out24hUsd || 0;
    // EXCLUDE yield-bearing wallets (LP/staking/reward) — a probe is a PURE deposit→withdraw round-trip with
    // no yield activity. This is what separates an exploit rehearsal from a legit LP/staker collecting rewards.
    return !w.yield && inn > 0 && out > 0 && inn <= PROBE_MAX && out <= PROBE_MAX && Math.abs(inn - out) <= PROBE_NET && w.firstSeen != null && (nowUnix - w.firstSeen) <= PROBE_FRESH;
  }).map((w) => ({ wallet: w.wallet, in: r2(w.in24hUsd || 0), out: r2(w.out24hUsd || 0), events: w.events, firstSeenAgoSec: Math.max(0, nowUnix - w.firstSeen) }));
  ...
  for (const c of custodies) {
    ...
    // Require the independent feed to be CONFIRMED fresh before it can drive a breach: stale mark, closed
    // session, MISSING publishTime (freshness unverifiable — e.g. a feed still warming up at boot), or a
    // stale publishTime → status "inactive" (muted), never breach. This closes the startup-warmup false
    // positive (a half-loaded cross-check value compared against a fresh mark) — a deviation only ever fires
    // when BOTH sides are provably fresh, which a real oracle manipulation always is.
  ...
  return { global, sides, internalFlows, tokens, wallets, oracle, dataCoverageSec, probes };
}
...
function ruleStates(ev) {
  ...
  // fresh-recipient: a large outflow (≥$10k) to a wallet first seen only within the last hour, measured
  // over the FULL retained buffer. `warm` suppresses these rules until the buffer covers ≥1h, so a
  // wallet isn't called "fresh" merely because our data doesn't reach back far enough (warmup guard).
  const warm = ev.dataCoverageSec == null || ev.dataCoverageSec >= 3600;
  for (const w of ev.wallets) if (warm && w.status === "ok" && (w.out1hUsd || 0) >= 10000 && w.firstSeenAgoSec != null && w.firstSeenAgoSec <= 3600) m[`freshwallet:${w.wallet}`] = { status: "warn", detail: `$${w.out1hUsd} out to a wallet first seen ${Math.round(w.firstSeenAgoSec / 60)}m ago` };
  ...
  // baseline EXCLUDES the current hour ((out24h − out1h)/23) so the multiple isn't diluted by the very
  // hour being tested — otherwise "≥5×" would actually trip at ~21% of daily outflow.
  const g = ev.global; if (g && (g.out1hUsd || 0) >= 5000) { const trailingAvg = ((g.out24hUsd || 0) - (g.out1hUsd || 0)) / 23; if (trailingAvg > 0 && g.out1hUsd > 5 * trailingAvg) m["acceleration"] = { status: "warn", detail: `outflow this hour ($${g.out1hUsd}) is ${(g.out1hUsd / trailingAvg).toFixed(1)}× the trailing-23h hourly average` }; }
  // new-wallet burst: multiple wallets first seen within the last hour ALL receiving outflow at once —
  // a coordinated-exfil pattern. Warmup-gated like the fresh-recipient rule.
  const freshRecipients = warm ? ev.wallets.filter((w) => w.firstSeenAgoSec != null && w.firstSeenAgoSec <= 3600 && (w.out1hUsd || 0) > 0) : [];
  ...
  // ---- rule: unpriced-outflow blind spot ----
  // A vault whose token has no priced oracle mark contributes $0 to every USD cap AND its drawdown guard is
  // muted (net USD = 0), so it could be drained invisibly. Surface any material unpriced outflow for review.
  const unpricedTok = (ev.tokens || []).filter((t) => (t.unpriced1h || 0) > 0 && (t.out1hAmt || 0) > 0 && (t.out1hUsd || 0) === 0);
  if (unpricedTok.length) m["unpriced-outflow"] = { status: "warn", detail: `${unpricedTok.length} vault(s) had UNPRICED outflow this hour — no oracle mark, so $0 in the USD caps and drawdown muted: ${unpricedTok.slice(0, 4).map((t) => `${t.symbol} ${(t.out1hAmt || 0).toFixed(2)}`).join(", ")} — verify manually` };
  // NOTE: the raw probe-cluster is NOT alerted here. Tiny deposit→withdraw round-trips are indistinguishable
  // from legit resumption testing at the pattern level — so ev.probes is surfaced for the dashboard, but the
  // ALARM is decided in sentinel.checkProbeCluster(), which verifies a COMMON FUNDING SOURCE on-chain first
  // (a single wallet funding ≥3 probe wallets = the coordinated-rehearsal fingerprint). Proof, or silence.
  // ---- rule: entitlement anomaly (the $1-in / $98k-out over-withdrawal fingerprint) ----
  // A wallet that WITHDRAWS far more than it just DEPOSITED, within the same window, is the exact
  // signature of a broken-entitlement / over-withdrawal exploit — you cannot legitimately take out 50×
  // what you put in within hours. We REQUIRE a recent same-window deposit (in24h > 0) so this never
  // false-flags a legitimate withdrawal of funds deposited days ago (those carry in24h ≈ 0). A $10k
  // materiality floor keeps dust out. This is the strongest single signal we have — the class the
  // conservation proof is blind to (it measures movement, not entitlement) — so it stands alone as CRITICAL.
  // Withdrawal basis is the LAST HOUR (out1h) so this fires on a LIVE over-withdrawal and does not keep
  // re-flagging a historical one that has already been handled but is still inside the 24h buffer. Deposit
  // basis stays 24h (in24h) so a deposit made hours before the withdrawal still anchors the ratio.
  for (const w of ev.wallets) {
    const out = w.out1hUsd || 0, inn = w.in24hUsd || 0;
    if (out >= 10000 && inn > 0 && out > 50 * inn) {
      m[`entitlement:${w.wallet}`] = { status: "breach", severity: "critical", detail: `⚠ CRITICAL — entitlement anomaly: withdrew $${r2(out)} this hour after depositing only $${r2(inn)} (24h) — ${Math.round(out / inn)}× more out than in, over-withdrawal fingerprint` };
    }
  }
  // ---- COMPOSITE THREAT SCORE ----
  // Correlate the independent signals per wallet. Any single signal is a soft warn (above); when 2+
  // converge on the SAME wallet the pattern is no longer noise — escalate to one loud CRITICAL alert
  // and suppress the individual warns for that wallet (dedup). All from real evaluated flow.
  const accel = !!m["acceleration"];
  for (const w of ev.wallets) {
    if (!((w.out1hUsd || 0) >= 1000)) continue; // only score wallets that actually moved material funds
    // HARD signals prove an entitlement/authority breach on their own merits — over the absolute cap, an
    // over-withdrawal fingerprint, or a wallet we ALREADY proved bad. SOFT signals (concentration %,
    // buffer-freshness, velocity spike) are RELATIVE artifacts of a quiet hour: during low activity ONE
    // legitimate withdrawal is naturally ~100% of the total and looks like a "spike", and a long-lived
    // wallet whose only outflow in our short buffer is recent looks "fresh" though it is not. Soft signals
    // may CORROBORATE a hard one, but they can never carry a CRITICAL drain verdict alone.
    const hard = [], soft = [];
    if (w.status !== "ok") hard.push(`over per-wallet cap (${w.utilization != null ? Math.round(w.utilization * 100) : "?"}%)`);
    if ((w.out1hUsd || 0) >= 10000 && (w.in24hUsd || 0) > 0 && (w.out1hUsd || 0) > 50 * (w.in24hUsd || 0)) hard.push(`withdrew ${Math.round((w.out1hUsd || 0) / (w.in24hUsd || 0))}× its deposit`);
    if (w.watched) hard.push("on watchlist");
    if ((w.out1hUsd || 0) >= 5000 && w.pctOfGlobal1h != null && w.pctOfGlobal1h >= 75) soft.push(`${w.pctOfGlobal1h}% of all outflow`);
    if (warm && (w.out1hUsd || 0) >= 10000 && w.firstSeenAgoSec != null && w.firstSeenAgoSec <= 3600) soft.push(`fresh wallet (${Math.round(w.firstSeenAgoSec / 60)}m old)`);
    if (accel && (w.out1hUsd || 0) >= 10000) soft.push("during an outflow spike");
    const signals = [...hard, ...soft];
    // CRITICAL requires ≥1 HARD signal corroborated by ≥1 more (hard or soft). Pure soft convergence is a
    // quiet-hour artifact, not a drain → at most a review WARN (never a red BREACH on relative signals only).
    if (hard.length >= 1 && signals.length >= 2) {
      m[`threat:${w.wallet}`] = { status: "breach", severity: "critical", detail: `⚠ CRITICAL — coordinated drain pattern: $${w.out1hUsd} out with ${signals.length} risk signals [${signals.join("; ")}]` };
      // dedup: fold ALL of this wallet's individual signals — including the hard per-wallet cap breach —
      // into the one CRITICAL, so it doesn't double-alert.
      delete m[`concentration:${w.wallet}`];
      delete m[`freshwallet:${w.wallet}`];
      delete m[`watch:${w.wallet}`];
      delete m[`wallet:${w.wallet}`];
      delete m[`entitlement:${w.wallet}`];
    } else if (soft.length >= 2) {
      // relative-signal convergence with NO hard proof → honest yellow "review", not a red drain claim.
      m[`elevated:${w.wallet}`] = { status: "warn", detail: `elevated concentrated outflow — $${w.out1hUsd} with ${soft.length} relative signals [${soft.join("; ")}] but no over-withdrawal or cap breach — likely a quiet-hour artifact, review` };
      delete m[`concentration:${w.wallet}`];
    }
  }
  return m;
