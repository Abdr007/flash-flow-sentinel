...
function ruleStates(ev) {
  ...
  // ---- rule: probe-cluster (exploit REHEARSAL) — ≥3 fresh wallets running tiny deposit→withdraw round-trips ----
  if (ev.probes && ev.probes.length >= 3) m["probe-cluster"] = { status: "breach", severity: "high", detail: `⚠ ${ev.probes.length} wallets ran tiny deposit→withdraw round-trips (exploit-rehearsal pattern) — ${ev.probes.slice(0, 4).map((p) => p.wallet.slice(0, 6) + "…($" + p.out + ")").join(", ")}${ev.probes.length > 4 ? " …" : ""}` };
