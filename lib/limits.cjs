...
const DEFAULT_LIMITS = {
  ...
  // Named wallet watchlist: any wallet here is flagged on ANY outflow in the window (even under the cap),
  // so wallets of interest from a prior incident are watched regardless of size. Seeded with the wallets
  // from the 2026-07-21 large-withdrawal review (the fresh $98k recipient + a repeat withdrawer).
  watchWallets: [
    "BjqHB51NRPLr4kCTrZyeqY6tehgACks3kx4kpGw3nWej",
    "FK9VWacDKwGD5G5RkejtsU1C7MgZzQiFZcpM3QpkzkRR",
  ],
...
function evaluate(nowUnix, events, failures, custodies, balances, marks, markTimes, lazerMarks, pyth, cfg, authority) {
  ...
  const wallets = Object.values(wal).map((w) => {
    ...
    return { wallet: w.wallet, watched: (cfg.watchWallets || []).includes(w.wallet), out1hUsd: r2(w.out1hUsd), out24hUsd: r2(w.out24hUsd), events: w.events, tokens: [...w.tokens], pctOfGlobal1h: out1.usd > 0 ? r2((w.out1hUsd / out1.usd) * 100) : null, limitUsdPerHour: wLimit, utilization: util == null ? null : r2(util * 100) / 100, status: status(util, cfg.warnFraction) };
