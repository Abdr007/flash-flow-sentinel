...
function evaluate(nowUnix, events, failures, custodies, balances, marks, markTimes, lazerMarks, pyth, cfg, authority) {
  ...
  for (const e of events) {
    ...
    if (e.wallet && (!authority || e.wallet !== authority)) {
      const w = wal[e.wallet] || (wal[e.wallet] = { wallet: e.wallet, out1hUsd: 0, out24hUsd: 0, in1hUsd: 0, in24hUsd: 0, events: 0, tokens: new Set(), firstSeen: walletFirstSeen[e.wallet] != null ? walletFirstSeen[e.wallet] : e.blockTime });
  ...
  const probes = Object.values(wal).filter((w) => {
    ...
    return inn > 0 && out > 0 && inn <= PROBE_MAX && out <= PROBE_MAX && Math.abs(inn - out) <= PROBE_NET && w.firstSeen != null && (nowUnix - w.firstSeen) <= PROBE_FRESH;
