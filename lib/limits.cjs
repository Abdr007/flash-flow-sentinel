...
function ruleStates(ev) {
  ...
  for (const w of ev.wallets) {
    const out = w.out24hUsd || 0, inn = w.in24hUsd || 0;
    if (out >= 10000 && inn > 0 && out > 50 * inn) {
      m[`entitlement:${w.wallet}`] = { status: "breach", severity: "critical", detail: `⚠ CRITICAL — entitlement anomaly: withdrew $${r2(out)} after depositing only $${r2(inn)} in the same window (${Math.round(out / inn)}× more out than in) — over-withdrawal fingerprint` };
    }
  }
  ...
  for (const w of ev.wallets) {
    ...
    if ((w.out24hUsd || 0) >= 10000 && (w.in24hUsd || 0) > 0 && (w.out24hUsd || 0) > 50 * (w.in24hUsd || 0)) signals.push(`withdrew ${Math.round((w.out24hUsd || 0) / (w.in24hUsd || 0))}× its deposit`);
    if (signals.length >= 2) {
