...
function ruleStates(ev) {
  ...
  for (const w of ev.wallets) {
    ...
    const signals = [];
    if (w.status !== "ok") signals.push(`over per-wallet cap (${w.utilization != null ? Math.round(w.utilization * 100) : "?"}%)`);
    if ((w.out1hUsd || 0) >= 5000 && w.pctOfGlobal1h != null && w.pctOfGlobal1h >= 75) signals.push(`${w.pctOfGlobal1h}% of all outflow`);
    if (warm && (w.out1hUsd || 0) >= 10000 && w.firstSeenAgoSec != null && w.firstSeenAgoSec <= 3600) signals.push(`fresh wallet (${Math.round(w.firstSeenAgoSec / 60)}m old)`);
    if (w.watched) signals.push("on watchlist");
    if (accel && (w.out1hUsd || 0) >= 10000) signals.push("during an outflow spike");
    if ((w.out1hUsd || 0) >= 10000 && (w.in24hUsd || 0) > 0 && (w.out1hUsd || 0) > 50 * (w.in24hUsd || 0)) signals.push(`withdrew ${Math.round((w.out1hUsd || 0) / (w.in24hUsd || 0))}× its deposit`);
    if (signals.length >= 2) {
