...
function evaluate(nowUnix, events, failures, custodies, balances, marks, markTimes, lazerMarks, pyth, cfg, authority) {
  ...
  for (const e of events) {
    ...
    if (isOut) {
      const w = wal[e.wallet] || (wal[e.wallet] = { wallet: e.wallet, out1hUsd: 0, out24hUsd: 0, events: 0, tokens: new Set() });
      w.out24hUsd += usd || 0;
  ...
  const tokens = Object.values(tok).map((t) => {
    ...
    const dUtil = cfg.vaultDrawdownPctPerHour > 0 ? drawdownPct / cfg.vaultDrawdownPctPerHour : null;
  ...
  const wallets = Object.values(wal).map((w) => {
    ...
    return { wallet: w.wallet, out1hUsd: r2(w.out1hUsd), out24hUsd: r2(w.out24hUsd), events: w.events, tokens: [...w.tokens], pctOfGlobal1h: out1.usd > 0 ? r2((w.out1hUsd / out1.usd) * 100) : null, limitUsdPerHour: wLimit, utilization: util == null ? null : r2(util * 100) / 100, status: status(util, cfg.warnFraction) };
  ...
  for (const c of custodies) {
    ...
    if (mark == null || !p) { oracle.push({ ...base, pythUsd: p ? p.price : null, deviationPct: null, status: "stale", pythSymbol: feed.pythSymbol, source: feed.source || null }); continue; }
    // A mark that is not updating (paused/closed market — provable via on-chain publish_time)
    // is NOT compared: a frozen mark drifting from a live feed is expected, not an attack.
    // The deviation guard fires only on FRESH marks — a live manipulated feed trips instantly.
    if ((markAgeSec != null && markAgeSec > 600) || p.marketSession === "closed") {
