...
// The single FAF token backing FLP across all pools (verified: every mainnet pool shares this tokenMint).
...
async function fetchPoolConfigSymbols(cluster = "mainnet-beta") {
  ...
  for (const p of j.pools || []) {
    ...
    // pool.tokenMint is the FAF token backing FLP — but only label the KNOWN FAF mint "FAF". Blindly
    // labeling every pool's tokenMint "FAF" would mislabel any future pool whose underlying differs.
    if (p.tokenMint === FAF_MINT) addKV(p.tokenMint, "FAF");
  }
