...
async function fetchFlashLazerPrices() {
  ...
  for (const [sym, p] of Object.entries(j || {})) {
    // reject null / non-finite / non-positive prices — a 0 or negative reference would make the
    // oracle-deviation guard divide by zero (|mark-0|/0 = Infinity) and fire a FALSE breach alarm.
    if (!p || p.priceUi == null || !Number.isFinite(Number(p.priceUi)) || Number(p.priceUi) <= 0) continue;
    out[sym] = {
