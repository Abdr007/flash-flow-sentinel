...
const IX_KIND = [
  ...
  [/StakeReward|CollectTokenReward/i, "REWARDS"], [/Rebate/i, "REBATE"], [/CollectRevenue/i, "REVENUE"],
...
function sideOf(e, kind) {
  ...
  switch (kind) {
    ...
    case "REWARDS": case "REBATE": case "FEES": case "REVENUE": return "protocol";
...
async function decodeFlow(mainRpc, sigInfo, cust, markUsd) {
  ...
  const ixNames = [...new Set(logs.map((l) => (l.match(/^Program log: Instruction: (\w+)/) || [])[1]).filter(Boolean))];
  // every programId invoked in this vault-touching tx (top-level + CPIs), for the new-program detector.
  const programs = [...new Set(logs.map((l) => (l.match(/^Program ([1-9A-HJ-NP-Za-km-z]{32,44}) invoke/) || [])[1]).filter(Boolean))];
  ...
  return {
    ...
    ix: ixNames, kind: classify(ixNames, direction), programs,
