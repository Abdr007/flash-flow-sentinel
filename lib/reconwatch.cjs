...
async function fetchCensusFull(url = CENSUS_URL, timeoutMs = 45000) {
  ...
  if (!r.ok) throw new Error(`census ${r.status}`);
  const j = await r.json();
  return { recon: parseReconciliation(j), inv: parseInvariants(j), backing: parseCustodyBacking(j) };
}

// Per-custody RAW backing from custodyMap. withdrawableResidualHuman = (vault + receivable) − (owned + payable)
// in TOKEN units — the exact solvency buffer for that one custody, and MARK-FREE (owned is realized token units;
// unrealized PnL never touches it). This lets the sentinel reconcile each custody's buffer against that custody's
// own base outflow (see checkCustodyBacking) — precise, attributable, and immune to the mark confound that limits
// the USD aggregate. Returns [] if the census predates custodyMap (caller falls back to the USD buffer).
function parseCustodyBacking(j) {
  const cm = j && Array.isArray(j.custodyMap) ? j.custodyMap : null;
  if (!cm) return [];
  return cm.filter((r) => r && !r.isVirtual && r.tokenAccount).map((r) => ({
    custody: r.custodyAccount, vault: r.tokenAccount, pool: r.pool, symbol: r.symbol, decimals: r.decimals, isStable: !!r.isStable,
    // withdrawableResidualHuman is the mark-free token-unit buffer; residualHuman is the same quantity (both = vault − net)
    residualHuman: Number(r.withdrawableResidualHuman != null ? r.withdrawableResidualHuman : r.residualHuman),
    markPrice: Number.isFinite(r.markPrice) ? r.markPrice : (r.isStable ? 1 : null),
    vaultStatus: r.vaultStatus, solvent: r.withdrawableSolvent !== false,
  })).filter((r) => Number.isFinite(r.residualHuman));
}
...
module.exports = { fetchReconciliation, fetchCensusFull, parseReconciliation, parseInvariants, parseCustodyBacking, diffMismatches, seed, solvencyStep, describe, CENSUS_URL };
