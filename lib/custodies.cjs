...
// Merge live mint→symbol metadata (from Flash's pool-config manifest / API registry) into the static
// map so EVERY resolution path below shows a real symbol instead of a mint prefix. Static entries win
// (they carry curated display casing like "Bonk"); live entries only fill mints the static map lacks.
function mergeSymbols(byMint) {
  if (!byMint) return 0;
  let added = 0;
  for (const [mint, meta] of Object.entries(byMint)) {
    if (!meta || !meta.symbol) continue;
    if (!SYMBOLS[mint]) { SYMBOLS[mint] = meta; added++; }
  }
  return added;
}

// Current best symbol for a mint (after merges). Used to RE-LABEL persisted flow events whose symbol
// was baked in at capture time before the live symbol map was merged — so the transaction feed shows a
// real symbol instead of a stale mint prefix. Returns null when the mint is genuinely unknown.
function symbolForMint(mint) {
  return (mint && SYMBOLS[mint] && SYMBOLS[mint].symbol) || null;
}
...
module.exports = { PROG, mergeSymbols, symbolForMint, scanCustodies, scanMarkets, scanNamedVaults, describeVault, sweepAuthority, fetchMarks, fetchVaultBalances };
