...
function sendSecurityAlert(text) {
  const tok = process.env.TELEGRAM_BOT_TOKEN, chat = process.env.SECURITY_CHAT_ID || process.env.OPERATOR_CHAT_ID || process.env.TELEGRAM_CHAT_ID;
  if (!tok || !chat) return Promise.resolve(null);
  const nowMs = Date.now();
  for (const [k, ts] of _recentSecText) if (nowMs - ts > 300000) _recentSecText.delete(k);
  if (_recentSecText.has(text)) return Promise.resolve(null); // never repeat the same security alert within 5 min
  _recentSecText.set(text, nowMs);
  return post(`https://api.telegram.org/bot${tok}/sendMessage`, { chat_id: chat, text, disable_web_page_preview: true });
}
