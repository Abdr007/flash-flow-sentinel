...
async function sendSecurityAlert(text) {
  // EVERY alarm routes here → the operator's PRIVATE DM first (OPERATOR_CHAT_ID), then any dedicated security
  // channel, then the main chat as a last resort. Never the public channel unless nothing else is configured.
  const tok = process.env.TELEGRAM_BOT_TOKEN, chat = process.env.OPERATOR_CHAT_ID || process.env.SECURITY_CHAT_ID || process.env.TELEGRAM_CHAT_ID;
...
module.exports = { deliverAlert, heartbeat, sendTelegram, sendOperator, sendSecurityAlert, sendWithdrawalNotice, sendPhoto, chartUrlForAlert, channelsConfigured };
