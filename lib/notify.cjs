...
function sendWithdrawalNotice(text) {
  const tok = process.env.TELEGRAM_BOT_TOKEN, chat = process.env.WITHDRAWAL_CHAT_ID || process.env.OPERATOR_CHAT_ID || process.env.TELEGRAM_CHAT_ID;
...
module.exports = { deliverAlert, heartbeat, sendTelegram, sendOperator, sendWithdrawalNotice, sendPhoto, chartUrlForAlert, channelsConfigured };
