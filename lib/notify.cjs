  ...
  if (m) {
    ...
    config = {
      ...
      options: {
        ...
        title: { display: true, text: `${t.pool}/${t.symbol} — on-chain flow (USD)  ·  drawdown 1h ${t.drawdownPct1h}%`, fontColor: "#e8f2ea" },
    ...
    const rows = tokens
      ...
      .sort((x, y) => (y.in24hUsd + y.out24hUsd) - (x.in24hUsd + x.out24hUsd))
      .slice(0, 18);
...
function sendPhoto(photoUrl, caption) {
  ...
  if (!tok || !chat || !photoUrl) return Promise.resolve();
...
function deliverAlert(a, cfg) {
  ...
  if (tgTok && tgChat) {
    ...
    if (wantsChart) {
      chartUrlForAlert(a, cfg.tokens).then((url) => {
        if (url) sendPhoto(url, line);
        else post(`https://api.telegram.org/bot${tgTok}/sendMessage`, { chat_id: tgChat, text: line, disable_web_page_preview: true });
      });
    } else {
