...
function diffGovernance(prev, cur) {
  ...
  // program redeploy — both slots must be real numbers
  if (prev.lastDeploySlot != null && cur.lastDeploySlot != null && prev.lastDeploySlot !== cur.lastDeploySlot)
    out.push({ key: "gov:program-deploy", severity: "critical", detail: `program REDEPLOYED (slot ${prev.lastDeploySlot} → ${cur.lastDeploySlot}) — verify the deploy was authorized` });
